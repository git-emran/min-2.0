    ...
    */
    view.webContents.on('did-start-navigation', function (event) {
      if (event.isMainFrame && !event.isSameDocument) {
        const currentId = getTabIDFromWebContents(view.webContents)
        const currentState = currentId && viewStateMap[currentId]

        if (!currentId || !currentState) {
          return
        }

        const hasJS = currentState.hasJS
        ...
        if (hasJS !== shouldHaveJS) {
          setTimeout(function () {
            ...
            destroyView(currentId)
            createView(
              null,
              currentId,
              Object.assign({}, currentState.webPreferences, { javascript: shouldHaveJS }),
              currentState.boundsString,
              currentState.events
            )
            loadURLInView(currentId, event.url, currentWindow)

            if (currentWindow) {
              setView(currentId, getWindowWebContents(currentWindow))
              focusView(currentId)
            }
          }, 0)
        }
...
ipc.on('callViewMethod', function (e, data) {
  ...
  try {
    ...
    if (data.method === 'canGoBack') {
      result = webContents.navigationHistory.canGoBack()
    } else if (data.method === 'canGoForward') {
      result = webContents.navigationHistory.canGoForward()
    } else if (data.method === 'goBack') {
      result = webContents.navigationHistory.goBack()
    } else if (data.method === 'goForward') {
      result = webContents.navigationHistory.goForward()
    } else if (data.method === 'canGoToOffset') {
      result = webContents.navigationHistory.canGoToOffset(...data.args)
    } else if (data.method === 'goToOffset') {
      result = webContents.navigationHistory.goToOffset(...data.args)
    } else {
...
ipc.handle('getNavigationState', function (e, id) {
  if (!viewMap[id]?.webContents) {
    return null
  }

  return {
    canGoBack: viewMap[id].webContents.navigationHistory.canGoBack(),
    canGoForward: viewMap[id].webContents.navigationHistory.canGoForward()
  }
})
