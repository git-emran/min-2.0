...
async function showSearchbarPlaceResults (text, input, inputFlags, pluginName = 'places') {
  ...
  results.forEach(function (result, index) {
    ...
    function onSwitchClick (task) {
      ...
      browserUI.switchToTab(task.tabs.find(tab => urlParser.removeTextFragment(tab.url) === urlComparisonBase).id)
    }

    const urlComparisonBase = urlParser.removeTextFragment(url)
    const matchingTasks = tasks.filter(task => task.tabs.some(tab => urlParser.removeTextFragment(tab.url) === urlComparisonBase)).sort((a, b) => {
