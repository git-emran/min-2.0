...
function addTab (tabId = tabs.add(), options = {}) {
  ...
  */
  ...
  const selectedTab = tabs.getRaw(selectedTabId)
  const newTab = tabs.getRaw(tabId)

  if (!options.openInBackground && selectedTab && !selectedTab.url && ((!selectedTab.private && newTab.private) || newTab.url)) {
...
function setWindowTitle () {
  ...
  const tab = task.tabs.getRaw(task.tabs.getSelected())
...
function switchToTask (id) {
  ...
  if (taskData.tabs.count() > 0) {
    ...
    if (!selectedTab) {
      selectedTab = taskData.tabs.getLatestActivityTab()?.id
    }
