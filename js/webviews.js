...
function captureCurrentTab (options) {
  const tabId = options?.tabId || tabs.getSelected()
  if (!tabId || webviews.selectedId !== tabId) {
    return
  }
...
function onPageLoad (tabId) {
  ...
  if (tabId === tabs.getSelected()) {
    clearTimeout(webviews.captureTimeouts[tabId])
    webviews.captureTimeouts[tabId] = setTimeout(function () {
      delete webviews.captureTimeouts[tabId]
      // preview captures are non-critical, so let the page settle before spending time on them
...
const webviews = {
  ...
  hasViewForTab: function (tabId) {
    ...
    const tab = task.tabs.getRaw(tabId)
    return !!(tab && tab.hasWebContents)
  },
  bindEvent: function (event, fn) {
    if (!webviews.events[event]) {
      webviews.events[event] = []
      webviews.eventNames.push(event)
    }

    webviews.events[event].push(fn)
  },
  unbindEvent: function (event, fn) {
    ...
    for (var i = 0; i < webviews.events[event].length; i++) {
      if (webviews.events[event][i] === fn) {
        webviews.events[event].splice(i, 1)
        i--
      }
    }

    if (webviews.events[event].length === 0) {
      delete webviews.events[event]
      webviews.eventNames = webviews.eventNames.filter(name => name !== event)
    }
  },
  emitEvent: function (event, tabId, args) {
    ...
    const listeners = webviews.events[event]
    if (!listeners) {
      return
    }

    listeners.slice().forEach(function (listener) {
      listener.apply(webviews, [tabId].concat(args))
    })
  },
  bindIPC: function (name, fn) {
    if (!webviews.IPCEvents[name]) {
      webviews.IPCEvents[name] = []
    }

    webviews.IPCEvents[name].push(fn)
  },
  ...
  requestPlaceholder: function (reason) {
    ...
    if (webviews.placeholderRequests.length >= 1) {
      ...
      var selectedTask = tasks.getTaskContainingTab(webviews.selectedId)
      var associatedTab = selectedTask && selectedTask.tabs.getRaw(webviews.selectedId)
      var img = associatedTab && associatedTab.previewImage
      if (img) {
  ...
  goBackIgnoringRedirects: async function (id) {
    ...
    if (!navHistory || navHistory.entries.length === 0) {
      return
    }
  ...
  getNavigationState: function (id) {
    return ipc.invoke('getNavigationState', id)
  }
}
...
ipc.on('view-ipc', function (e, args) {
  ...
  const handlers = webviews.IPCEvents[args.name]
  if (!handlers) {
    return
  }

  handlers.forEach(function (handler) {
    handler(args.id, [args.data], args.frameId, args.frameURL)
  })
})
